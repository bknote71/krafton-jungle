...
{
    for (char *bp = NEXT_BLKP(heap_list); bp != NULL; bp = NEXT_BLKP(bp))
    {
        int bsize = GET_SIZE(HDRP(bp));
        int alloc = GET_ALLOC(HDRP(bp));
        if (bsize == 0 && alloc == 1)
            return NULL;
        if (bsize >= size && alloc == 0)
            return bp;
    }
    return NULL;
}
...
int mm_init(void)
{
    ...
    PUT(heap_list + (1 * WSIZE), PACK(DSIZE, 1)); // prolog header
    PUT(heap_list + (2 * WSIZE), PACK(DSIZE, 1)); // prolog header
