...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    size_t new_size = ALIGN(size + DSIZE); // prev, next
    ...
    size_t bsize = org_size;
    if (!prev_alloc)
    {
        bp = PREV_BLKP(ptr);
        bsize += GET_SIZE(FTRP(bp));
    }

    if (!next_alloc)
    {
        ep = NEXT_BLKP(ptr);
        bsize += GET_SIZE(HDRP(ep));
    }
    ...
    if (bsize >= new_size)
    {
        ...
        PUT(FTRP(ep), PACK(bsize, 1));
        if (bp != ptr)
            memmove(bp, ptr, new_size);
        return bp;
    }
