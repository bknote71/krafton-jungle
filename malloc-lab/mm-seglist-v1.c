...
#define GET_HD_BLK_SIZE(p) GET_SIZEå(HDRP(p))
...
{
    char *p;
    for (int index = INDEX(size); index < LIMIT; ++index)
    {
        if (seg_array[index] != NULL)
        {
            p = seg_array[index];
            for (; p != NULL && GET_ALLOC(HDRP(p)) == 0; p = SUCC(p))
            {
...
static void insert(void *bp)
{
    ...
    // printf("insert %d size this index %d\n", GET_SIZE(HDRP(bp)), index);
    SUCC(bp) = seg_array[index];
    PREV(seg_array[index]) = bp;
    PREV(bp) = NULL;
    seg_array[index] = bp;
}
